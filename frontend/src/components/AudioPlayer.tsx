...
import { Button, Slider, Box, Typography, Card, CardContent, CardMedia, List, ListItem, ListItemText, ListItemButton } from '@mui/material';
...
const AudioPlayer = () => {
  const [status, setStatus] = useState<AudioStatus>({ isPlaying: false, volume: 80, currentFilePath: null, progress: 0, duration: 0 });
  ...
  const handleStop = async () => {
    try {
      const response = await fetch('http://localhost:3001/api/audio/stop', {
        method: 'POST',
      });
      const data = await response.json();
      setStatus(data.status);
  ...
  const handleSeekCommitted = async (_event: any, value: number | number[]) => {
                    ...
                    <Card sx={{ width: '100%', mb: 1 }}>
                      <Box sx={{ display: 'flex' }}>
                        ...
                        <CardContent sx={{ flex: 1 }}>
                          ...
                          <Box sx={{ mt: 1 }}>
                            {status.isPlaying && currentSong?.id === song.id ? (
                              <Button
                                variant="contained"
                                size="small"
                                startIcon={<PauseIcon />}
                              ...
                              <Button
                                ...
                                disabled={status.isPlaying && currentSong?.id !== song.id}
                              >
                                Play
                              </Button>
                            )}
              ...
              <Box sx={{ mt: 1 }}>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                  <Typography variant="caption">
                    {formatTime(isDragging ? seekValue || 0 : status.progress || 0)}
                  </Typography>
                  <Typography variant="caption">
