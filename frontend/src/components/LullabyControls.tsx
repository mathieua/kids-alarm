...
const LullabyControls = () => {
    ...
    const [selectedMusicFile, setSelectedMusicFile] =
        useState<MusicFile | null>(null);

    useEffect(() => {
        const fetchMusicFiles = async () => {
            try {
                const response = await axios.get(
                    "http://localhost:3001/api/lullaby/music"
                );
                ...
                if (response.data.files.length > 0) {
                    ...
                    setSettings((prev) => ({
                        ...prev,
                        selectedMusic: response.data.files[0].id,
                    }));
                }
            } catch (error) {
    ...
    const handleStartLullaby = async () => {
        try {
            await axios.post(
                "http://localhost:3001/api/lullaby/start",
                settings
            );
            setIsPlaying(true);
        } catch (error) {
        ...
        >
            ...
            {selectedMusicFile && (
                <MusicPlayer
                    ...
                    isPlaying={isPlaying}
                    onPlayPause={
                        isPlaying ? handleStopLullaby : handleStartLullaby
                    }
                />
            )}
